...
def generate_large_corpus(num_docs=10000, avg_doc_len=100, vocab_size=1000, seed=42):
    ...
    print(
        f"Generating corpus with {num_docs} documents, avg length {avg_doc_len}, vocab size {vocab_size}"
    )
...
def test_consistency():
    ...
    consistent = all(
        scores == first_scores and indices == first_indices
...
def test_performance():
    ...
    for test_idx, (num_docs, avg_len, vocab_size) in enumerate(test_cases):
        ...
        text_corpus = generate_large_corpus(
            num_docs, avg_len, vocab_size, seed=42 + test_idx
        )
        ...
        query_tokens = list(token_to_id.keys())[
            :5
        ]  # Use first 5 tokens for consistency
        ...
        is_consistent = all(
            s == first_scores and t == first_top for s, t in consistent_results[1:]
        )
        ...
        print(
            f"  Total time: {corpus_gen_time + vocab_time + conversion_time + bm25_init_time:.3f}s"
        )
