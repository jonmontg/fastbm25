...
def test_document_order():
    ...
    for i in range(5):
        ...
        print(
            f"Run {i+1}: scores={[f'{s:.3f}' for s in scores]}, top_indices={top_indices}"
        )
    ...
    if all_identical:
        print(
            "✅ SUCCESS: All runs produce identical results - document order is preserved!"
        )
    else:
        print(
            "❌ FAILURE: Results differ between runs - document order is not preserved!"
        )
